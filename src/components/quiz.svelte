...
<div class="w-auto mx-4">
	...
	<ul class="my-4 flex flex-col space-y-4">
		{#each quiz.options as option, index (option)}
			<li class="max-w-full">
				<label
					for={`${quiz.id}-${index}`}
					class={`label cursor-pointer justify-start space-x-4 border px-4 w-full ${optionBadge(
						option
					)}`}
				>
					{#if !choices}
	...
	<div class="flex items-center">
		...
		>
		<a
			href={`https://github.com/raysca/revis3/edit/main/quizzes/${quiz.topic?.path}`}
			class="btn btn-link"
			target="_blank">Fix this question</a
		>
	</div>
